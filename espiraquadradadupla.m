% FSS do tipo espira quadrada dupla
clc;
clear
% Entrada de dados
f_inicial = 1;
...
B1  = 2.5*eff.*H(p,g1,lambda,teta); %calcula a susceptancia da Bc1
B2  = 2.5*eff.*H(p,g2,lambda,teta); %calcula a susceptancia da Bc2

    XL1(ii) = 2.*(d1./p).*(X1(ii).*X2(ii)./(X1(ii)+X2(ii))); %calcula a reatancia indutiva XLf1
    XL2(ii) = (d2./p).*X3(ii);
    BC1(ii) = 0.75.*B1(ii).*(d1./p);
    BC2(ii) = (d2./p).*(B1(ii).*B2(ii)./(B1(ii)+B2(ii)));
...
ct(ii)=10.*log10(4./(4+(y(ii).^2))); %coeficiente de transmissao 
 ...
 %comparação 
% Frequência
f1= xlsread('quadradadupla.xlsx',1,'A2:A1002');

% Transmissão
x1= xlsread('quadradadupla.xlsx',1,'B2:B1002');
